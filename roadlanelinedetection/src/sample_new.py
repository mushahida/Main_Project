...
def draw_lines(img, lines, color=[255, 0, 0], thickness=10):
    ...
    to that shown in P1_example.mp4).  Think about things like separating line segments by their
...
def lane_finding_pipeline(image):
    ...
    masked_img,vet = region_of_interest(img=canny_img, vertices=get_vertices(image))
    # print(vet)
...
# for image_path in list(os.listdir('test_images')):
    ...
    image = mpimg.imread(f'{path}')
    ax = fig.add_subplot(1, 2, 1,xticks=[], yticks=[])
    # plt.imshow(image)
    # ax.set_title("Input Image")
    # ax = fig.add_subplot(1, 2, 2,xticks=[], yticks=[])
    rr,v=lane_finding_pipeline(image)
